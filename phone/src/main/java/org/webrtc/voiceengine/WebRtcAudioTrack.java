...
public class WebRtcAudioTrack {
    ...
    private int InitPlayback(int sampleRate) {
    	DoLog("stream="+VoiceEngineConfig.sharedInstance().getTrackStream());
        ...
        try {
            _audioTrack = new AudioTrack(
                            VoiceEngineConfig.sharedInstance().getTrackStream(),
                            sampleRate,
                            AudioFormat.CHANNEL_OUT_MONO,
                            AudioFormat.ENCODING_PCM_16BIT,
        ...
        return _audioManager.getStreamMaxVolume(VoiceEngineConfig.sharedInstance().getTrackStream());
    }
    ...
    private int StartPlayback() {
		...
		if( _context != null ){
			...
			if( _audioManager != null ){
				DoLog("mode="+_audioManager.getMode());
				DoLog("speaker="+_audioManager.isSpeakerphoneOn());
				DoLog("microphone mute="+_audioManager.isMicrophoneMute());
				DoLog("music="+_audioManager.isMusicActive());
				DoLog("bluetoothSco="+_audioManager.isBluetoothScoOn());
				DoLog("bluetoothA2dp="+_audioManager.isBluetoothA2dpOn());
				DoLog("volume="+_audioManager.getStreamVolume(
					VoiceEngineConfig.sharedInstance().getTrackStream()));
			}
		}
    ...
    private int SetPlayoutVolume(int level) {
        ...
        if (_audioManager != null) {
            _audioManager.setStreamVolume(
				VoiceEngineConfig.sharedInstance().getTrackStream(), level, 0);
            retVal = 0;
        }
    ...
    final String logTag = "VoiceEngine";
