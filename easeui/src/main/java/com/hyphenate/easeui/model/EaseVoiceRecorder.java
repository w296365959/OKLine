...
public class EaseVoiceRecorder {
    ...
    public String startRecording(Context appContext) {
        ...
        try {
            ...
            // User.getVoicePath());
            //update by luoxiuxiu 170627 音频路径
            voiceFileName = getVoiceFileName(EMClient.getInstance().getCurrentUser());
//            PathUtil.getInstance().initDirs("ease","ease",appContext);
            File voiceFile = PathUtil.getInstance().getVoicePath();
            voiceFilePath = voiceFile.getAbsolutePath() + "/" + voiceFileName;

//            File mFolder = new File(PathUtil.getInstance().getVoicePath().getAbsolutePath());
            Timber.i("voicePath exists"+voiceFile.exists());
            if (!voiceFile.exists()) {
                voiceFile.mkdirs();
                Timber.i("voicePath exists"+voiceFile.exists());
            }

            file = new File(voiceFile, voiceFileName);
