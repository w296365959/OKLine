    ...
    private void setUpBaseView() {
        ...
        if (timestamp != null) {
            if (position == 0) {
            	...
            	// show time stamp if interval with last message is > 10 minute
                EMMessage prevMessage = (EMMessage) adapter.getItem(position - 1);
                if (prevMessage != null && DateUtil.isCloseEnough(message.getMsgTime(), prevMessage.getMsgTime())) {
        ...
        if(message.direct() == Direct.SEND){//本人
             User user = UserRepository.getInstance(getContext()).getUser();
            Glide.with(context).load(user.getAvatar()).diskCacheStrategy(DiskCacheStrategy.ALL).placeholder(R.drawable.ol_default_avatar).into(userAvatarView);

        }else{

            EaseUserUtils.setAvatarNick(context,message.getFrom(),userAvatarView,usernickView);
//            EaseUserUtils.setAvatarNick(context,message.getFrom(),userAvatarView,usernickView);
