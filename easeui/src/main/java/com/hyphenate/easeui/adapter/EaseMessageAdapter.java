...
import com.hyphenate.easeui.widget.chatrow.EaseDeliveryRow;
import com.hyphenate.easeui.widget.chatrow.EasePhoneRow;
...
public class EaseMessageAdapter extends BaseAdapter{
	...
	private static final int   MESSAGE_TYPE_RECV_PHONE = 14; //收到电话
	private static final int  MESSAGE_TYPE_SENT_PHONE = 15;//发送电话

    private static final int MESSAGE_TYPE_RECV_DELIVERY = 16; //对方
    private static final int MESSAGE_TYPE_SENT_DELIVERY = 17;//自己
	...
	public int getItemViewType(int position) {
		...
		if (message.getType() == EMMessage.Type.TXT) {
			...
			if(EaseConstant.MESSAGE_IS_PHONE_CALL.equals(message.getStringAttribute(EaseConstant.MESSAGE_ATTR_IS_PHPONE,""))){
				Log.v("hx1","itemviewTyoe-->phone");
				return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_PHONE : MESSAGE_TYPE_SENT_PHONE;
			}if (message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_DELIVERY, false)) {//快递
                return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_DELIVERY : MESSAGE_TYPE_SENT_DELIVERY;
            }
	...
	protected EaseChatRow createChatRow(Context context, EMMessage message, int position) {
        ...
        case TXT:
            ...
            }else if(message.getStringAttribute(EaseConstant.MESSAGE_ATTR_IS_PHPONE,"").equals(EaseConstant.MESSAGE_IS_PHONE_CALL)){
				chatRow=new EasePhoneRow(context, message, position, this);
				Log.v("hx1","createPhoneRow");
			}else if (message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_DELIVERY, false)) {////快递
                chatRow = new EaseDeliveryRow(context, message, position, this);
            }
			else{
