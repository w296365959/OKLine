...
public class OPayPasswordFragment extends Fragment implements View.OnClickListener {
    ...
    ImageView ivIcon;
    @BindView(R.id.tv_paypophint)
    TextView tvPaypophint;
    @BindView(R.id.tv_opaypwdcancel)
    TextView tvOpaypwdcancel;
    @BindView(R.id.psw_dot1)
    ImageView pswDot1;
    @BindView(R.id.psw_dot2)
    ImageView pswDot2;
    @BindView(R.id.psw_dot3)
    ImageView pswDot3;
    @BindView(R.id.psw_dot4)
    ImageView pswDot4;
    @BindView(R.id.psw_dot5)
    ImageView pswDot5;
    @BindView(R.id.psw_dot6)
    ImageView pswDot6;
    @BindView(R.id.ll_pwdface)
    LinearLayout llPwdface;
    @BindView(R.id.tv_forgetpwd)
    TextView tvForgetpwd;
    @BindView(R.id.btn_pwdkey1)
    TextView btnPwdkey1;
    @BindView(R.id.btn_pwdkey2)
    TextView btnPwdkey2;
    @BindView(R.id.btn_pwdkey3)
    TextView btnPwdkey3;
    @BindView(R.id.btn_pwdkey4)
    TextView btnPwdkey4;
    @BindView(R.id.btn_pwdkey5)
    TextView btnPwdkey5;
    @BindView(R.id.btn_pwdkey6)
    TextView btnPwdkey6;
    @BindView(R.id.btn_pwdkey7)
    TextView btnPwdkey7;
    @BindView(R.id.btn_pwdkey8)
    TextView btnPwdkey8;
    @BindView(R.id.btn_pwdkey9)
    TextView btnPwdkey9;
    @BindView(R.id.btn_pwdkeyblank)
    TextView btnPwdkeyblank;
    @BindView(R.id.btn_pwdkey0)
    TextView btnPwdkey0;
    @BindView(R.id.btn_pwdkeydel)
    ...
    public View onCreateView(final LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (view == null) {
            view = inflater.inflate(R.layout.fragment_opaysdk3, container, false);
...
//                    Utils.customToast(getContext(), "暂时不可用", Toast.LENGTH_SHORT).show();
            ...
            if (tvPaypophint!=null) {
                tvPaypophint.setText("请输入"+activity.selectedCard.cardName()+"密码");
            }
        }
        return view;
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_pwdkeydel:
                if (pwd.length() > 0) {
                    pwd.deleteCharAt(pwd.length() - 1);
                    setDotVisibility(list, pwd.length());
                }
                break;
            case R.id.btn_pwdkey0:
                pwd.append(0);
                checkPwd();
                break;
            case R.id.btn_pwdkey1:
                pwd.append(1);
                checkPwd();
                break;
            case R.id.btn_pwdkey2:
                pwd.append(2);
                checkPwd();
                break;
            case R.id.btn_pwdkey3:
                pwd.append(3);
                checkPwd();
                break;
            case R.id.btn_pwdkey4:
                pwd.append(4);
                checkPwd();
                break;
            case R.id.btn_pwdkey5:
                pwd.append(5);
                checkPwd();
                break;
            case R.id.btn_pwdkey6:
                pwd.append(6);
                checkPwd();
                break;
            case R.id.btn_pwdkey7:
                pwd.append(7);
                checkPwd();
                break;
            case R.id.btn_pwdkey8:
                pwd.append(8);
                checkPwd();
                break;
            case R.id.btn_pwdkey9:
                pwd.append(9);
                checkPwd();
                break;
        }
            ...
            /*开始检查密码*/
                Observable.create(new Observable.OnSubscribe<Boolean>() {
                    ...
                    public void call(Subscriber<? super Boolean> subscriber) {
                        try {
                            // TODO: 2017/4/15 写死olNo,停用密码比对
                            checkPayPwd = new CheckPayPwd().checkPayPwd(activity.user != null ? activity.user.getOlNo() : CardOpenRechargeFragment.OL_NO, EncryptUtils.password(pwdtext), "olApp", "OKLine");
                            ...
                            Observable.create(new Observable.OnSubscribe<Boolean>() {
                                ...
                                public void call(Subscriber<? super Boolean> subscriber) {
                                    try {
                                        ...
                                        String olNo = activity.user != null ? activity.user.getOlNo() : CardOpenRechargeFragment.OL_NO;
                                        Timber.tag(TAG).i("olNo: " + olNo);
                                        Timber.tag(TAG).i("orderNumber:" + activity.orderNumber);
                                        Timber.tag(TAG).i("tn:" + activity.orderTn);
                                        Timber.tag(TAG).i("bindId:" + activity.selectedCard.bindId());
                                        ...
                                        bankDebitResult = new CustomDebit().bankDebit(olNo, activity.orderNumber, activity.orderTn, activity.selectedCard.bindId(), activity.orderAmount, activity.merNo, activity.orderDesc, P_ID, P_CODE);
                                ...
                                public void onNext(Boolean aBoolean) {
                                    ...
                                    activity.bankDebitInfo = new BankDebitInfo(
                                            ...
                                            activity.cardId,
                                            activity.selectedCard.cardName(),
                                            activity.selectedCard.cardNo(),
