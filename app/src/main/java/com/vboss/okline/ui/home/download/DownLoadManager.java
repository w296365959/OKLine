...
package com.vboss.okline.ui.home.download;

import android.content.Context;
import android.content.SharedPreferences;


import com.vboss.okline.ui.home.download.dbcontrol.DataKeeper;
import com.vboss.okline.ui.home.download.dbcontrol.FileHelper;
import com.vboss.okline.ui.home.download.dbcontrol.bean.SQLDownLoadInfo;
...
public class DownLoadManager {
    ...
    private void recoverData(Context context, String userID){
        ...
        if (sqlDownloadInfoList.size() > 0) {
            ...
            Timber.tag("download").i("恢复下载数据  listSize :%s",listSize);
...
//        }
    ...
    public ArrayList<TaskInfo> getAllTask() {
        ...
        for (int i = 0; i < listSize; i++) {
            ...
            Timber.tag("download").i("isDownLoading() :%s",deletedownloader.isDownLoading());
            TaskInfo taskinfo = new TaskInfo();
            taskinfo.setFileName(sqldownloadinfo.getFileName());
            //modified by yuanshaoyu 2017-6-30:杀掉进程重新升级直接开始继续下载
            taskinfo.setOnDownloading(true);//deletedownloader.isDownLoading()
    ...
    public void startTask(String taskID) {
        int listSize = taskList.size();
        Timber.tag(DownLoadManager.class.getSimpleName()).i("list szie %s",listSize);
        for (int i = 0; i < listSize; i++) {
            DownLoader deletedownloader = taskList.get(i);
            if (deletedownloader.getTaskID().equals(taskID)) {
