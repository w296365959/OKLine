...
public class CardPasswordFragment extends Fragment implements View.OnClickListener {
    ...
    ImageView ivIcon2;
    @BindView(R2.id.tv_paypophint2)
    TextView tvPaypophint2;
    @BindView(R2.id.tv_paypopcancel2)
    TextView tvPaypopcancel2;
    @BindView(R2.id.bankpsw_dot1)
    ImageView bankpswDot1;
    @BindView(R2.id.bankpsw_dot2)
    ImageView bankpswDot2;
    @BindView(R2.id.bankpsw_dot3)
    ImageView bankpswDot3;
    @BindView(R2.id.bankpsw_dot4)
    ImageView bankpswDot4;
    @BindView(R2.id.bankpsw_dot5)
    ImageView bankpswDot5;
    @BindView(R2.id.bankpsw_dot6)
    ImageView bankpswDot6;
    @BindView(R2.id.ll_pwdface2)
    RelativeLayout llPwdface2;
    @BindView(R2.id.btn_bankpwdkey1)
    TextView btnBankpwdkey1;
    @BindView(R2.id.btn_bankpwdkey2)
    TextView btnBankpwdkey2;
    @BindView(R2.id.btn_bankpwdkey3)
    TextView btnBankpwdkey3;
    @BindView(R2.id.btn_bankpwdkey4)
    TextView btnBankpwdkey4;
    @BindView(R2.id.btn_bankpwdkey5)
    TextView btnBankpwdkey5;
    @BindView(R2.id.btn_bankpwdkey6)
    TextView btnBankpwdkey6;
    @BindView(R2.id.btn_bankpwdkey7)
    TextView btnBankpwdkey7;
    @BindView(R2.id.btn_bankpwdkey8)
    TextView btnBankpwdkey8;
    @BindView(R2.id.btn_bankpwdkey9)
    TextView btnBankpwdkey9;
    @BindView(R2.id.btn_bankpwdkeyblank)
    TextView btnBankpwdkeyblank;
    @BindView(R2.id.btn_bankpwdkey0)
    TextView btnBankpwdkey0;
    @BindView(R2.id.btn_bankpwdkeydel)
    RelativeLayout btnBankpwdkeydel;
    @BindView(R2.id.gv_bankkeyboard)
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (view == null) {
            view = inflater.inflate(R.layout.fragment_opaysdk4_assistant, null);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R2.id.btn_bankpwdkeydel:
                if (pwd.length() > 0) {
                    pwd.deleteCharAt(pwd.length() - 1);
                    setDotVisibility(list, pwd.length());
                }
                break;
            case R2.id.btn_bankpwdkey0:
                pwd.append(0);
                checkPwd();
                break;
            case R2.id.btn_bankpwdkey1:
                pwd.append(1);
                checkPwd();
                break;
            case R2.id.btn_bankpwdkey2:
                pwd.append(2);
                checkPwd();
                break;
            case R2.id.btn_bankpwdkey3:
                pwd.append(3);
                checkPwd();
                break;
            case R2.id.btn_bankpwdkey4:
                pwd.append(4);
                checkPwd();
                break;
            case R2.id.btn_bankpwdkey5:
                pwd.append(5);
                checkPwd();
                break;
            case R2.id.btn_bankpwdkey6:
                pwd.append(6);
                checkPwd();
                break;
            case R2.id.btn_bankpwdkey7:
                pwd.append(7);
                checkPwd();
                break;
            case R2.id.btn_bankpwdkey8:
                pwd.append(8);
                checkPwd();
                break;
            case R2.id.btn_bankpwdkey9:
                pwd.append(9);
                checkPwd();
                break;
            default:
